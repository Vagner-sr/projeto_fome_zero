...
with st.container():
    ...
    cidades = ( df.loc[:, ["City", 'Restaurant Name']].groupby('City')
               .count().sort_values('Restaurant Name', ascending = False).reset_index() )
    cidades.columns = ['City', 'Restaurants quantity']
    cidades = cidades.head(10)
    
    cidades = px.bar(cidades, x='City', y = 'Restaurants quantity', color = 'City')
    st.plotly_chart(cidades, use_container_width = True)

with st.container():
    col1, col2 = st.columns(2)
    with col1:
        st.markdown('#### Top 7 restaurantes com média acima de 4.0')
    
        restaurantes = ( df.loc[df['Aggregate rating'] > 4.0, ['Restaurant Name', 'Aggregate rating']]
                        .groupby('Restaurant Name').mean().sort_values('Aggregate rating', ascending = False).reset_index() )
        restaurantes = restaurantes.head(7)
    
        restaurantes = px.bar(restaurantes, x = 'Restaurant Name', y = 'Aggregate rating', color = 'Restaurant Name')
        st.plotly_chart(restaurantes, use_container_width = True)

    with col2:
        st.markdown('#### Top 7 restaurantes com média abaixo de 4.0')
...
with st.container():
    st.markdown('#### Top 10 cidades com mais tipos de culinárias distintas')
    
    cities = df.loc[:,['City', 'Cuisines']].groupby('City').nunique().sort_values('Cuisines', ascending = False).reset_index()
    cities = cities.head(10)
    cities = px.bar(cities, x= 'City', y = 'Cuisines', color = 'City')
    
    st.plotly_chart(cities, use_container_width = True)
